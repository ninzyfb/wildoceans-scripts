...
en_spring <- BIOMOD_EnsembleModeling(
  modeling.output = spring, # your models object
...
spring =
  BIOMOD_Projection(
    ...
    modeling.output = spring, # your modelling output object
...
en_spring = BIOMOD_EnsembleForecasting(
  EM.output = en_spring,
  projection.output = spring)

##### Pretty plots
library(rasterVis)
library(viridis)
en_preds = get_predictions(en_spring)
type = "Spring"
