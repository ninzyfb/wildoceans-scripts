...
mpas = shapefile(list.files(pattern ="SAMPAZ_OR_2020_Q3.shp" ,recursive = TRUE))
...
png(file=paste0("Outputs/solutions/","p",p+i,"_scenario",scenario,"_","target",target_group[i],"_",season_order[i],".png"), width=3000, height=3000, res=300)
...
dev.off()
# one with the mpas
png(file=paste0("Outputs/solutions/","p",p+i,"_scenario",scenario,"_","target",target_group[i],"_",season_order[i],"_withmpas.png"), width=3000, height=3000, res=300)
plot(list_solutions[[i]], col = c("grey90", "darkgreen"), main = paste("Scenario:",scenario,"Season:",season_order[i],"Target:",target_group[i]), sub = paste("penalty:", penalty,"problem:",p+i))
plot(mpas,add = TRUE)
dev.off()
}
